...
import defusedxml
defusedxml.defuse_stdlib()  # patches stdlib XML parsers to be safe against XML attacks
import time
import shutil
import tempfile
import argparse
...
SCA_account = os.environ.get('SCA_ACCOUNT', '')
SCA_username = os.environ.get('SCA_USERNAME', '')
SCA_password = os.environ.get('SCA_PASSWORD', '')

SCA_proxy = ''

SCA_high_threshold = 0 # block all
...
# Global setting for showing license issues only for direct packages
SHOW_LICENSE_ISSUES_ONLY_FOR_DIRECT = True
...
RESTRICTED_LICENSES = ["AGPL", "GPL"]  # You can extend this list as needed
    ...
    try:
        if os.path.exists(exclude_file):
            with open(exclude_file, 'r') as f:
                additional_packages = json.load(f)
                EXCLUDE_PACKAGES.extend(additional_packages)
                print(f"Loaded {len(additional_packages)} additional packages to exclude from {exclude_file}")
    except Exception as e:
        print(f"Error loading exclude packages from {exclude_file}: {e}")
...
def SCA_get_access_token():
    try:
        ...
        response = requests.post(SCA_auth_url, headers=headers, data=payload, verify=False, proxies=proxy_servers, timeout=30)
...
def SCA_create_project(project_name, access_token="", team_name=None):  # nosec B107
    if(not access_token):
    ...
    try:
        ...
        response = requests.request("POST", url, headers=headers, data=payload, proxies=proxy_servers, verify=False, timeout=30)
...
def SCA_get_project_id(project_name, access_token=""):  # nosec B107
    if(not access_token):
        access_token = SCA_get_access_token()
    ...
    try:
        ...
        response = requests.get(url, headers=headers, proxies=proxy_servers, verify=False, timeout=30)
        if response.status_code == 404:
            print(f"Project '{project_name}' not found - will create it")
            return ""
...
def SCA_get_project_latest_scan_id(project_name, access_token=""):  # nosec B107
    if(not access_token):
        access_token = SCA_get_access_token()
    ...
    try:
        ...
        response = requests.request("GET", url, headers=headers, data=payload, proxies=proxy_servers, verify=False, timeout=30)
        response_json = response.json()
    except Exception as e:
...
def SCA_get_upload_link(project_id, access_token):
    ...
    try:
        ...
        response = requests.post(url, headers=headers, json=payload, proxies=proxy_servers, verify=False, timeout=30)
        response.raise_for_status()  # Raise an error for bad responses
...
def SCA_upload_file(upload_link, zip_file_path, access_token=""):  # nosec B107
    if(not access_token):
        access_token = SCA_get_access_token()
    ...
    try:
        with open(zip_file_path, 'rb') as file:
            ...
            response = requests.put(upload_link, headers=headers, data=file, proxies=proxy_servers, verify=False, timeout=120)
            response.raise_for_status()  # Raise an error for bad responses
            # print('SCA_upload_file:', response.text)
    except requests.RequestException as e:
        print("Exception: Failed to upload file:", str(e))

def SCA_scan_zip(project_id, upload_file_url, access_token=""):  # nosec B107
    if(not access_token):
        access_token = SCA_get_access_token()
...
def SCA_get_scan_status(scan_id, access_token=""):  # nosec B107
    if(not access_token):
        access_token = SCA_get_access_token()
...
def SCA_get_report(project_name, report_type, access_token=""):  # nosec B107
    if(not access_token):
        access_token = SCA_get_access_token()
...
def get_vulnerable_packages_from_report(project_name, access_token=""):  # nosec B107
    try:
        if not access_token:
            access_token = SCA_get_access_token()
        ...
        try:
            ...
            if csv_filename in extracted_data:
                ...
                for _, row in packages_df.iterrows():
                    ...
                    # Process high severity vulnerabilities with risk score filtering
                    if row['Severity'] == 'high':
                        # Parse the risk score for filtering
                        parsed_risk_score = parse_risk_score(risk_score)
                        
                        # Apply risk score threshold filtering if it's configured
                        should_include_vulnerability = True
                        if RISK_SCORE_THRESHOLD is not None:
                            # Only include vulnerabilities that exceed the risk score threshold OR have no valid risk score
                            if parsed_risk_score is not None:
                                should_include_vulnerability = parsed_risk_score > RISK_SCORE_THRESHOLD
                            # If parsed_risk_score is None (N/A), we include it by default to be conservative
                        
                        if should_include_vulnerability:
                            vulnerabilities.append((package_name, package_version, dependency_type, risk_score, is_direct))
                    
                    # For license issues, check SHOW_LICENSE_ISSUES_ONLY_FOR_DIRECT setting
                    if found_due_to_license:
                        if SHOW_LICENSE_ISSUES_ONLY_FOR_DIRECT:
                            # Only add direct dependencies with license issues
                            if is_direct:
                            ...
                            # Add all license issues (both direct and transitive)
                            license_text = f" - Found due to {', '.join(found_due_to_license)} license"
                            license_issues.append((package_name, package_version, dependency_type + license_text, risk_score, is_direct))

                # Print vulnerabilities with risk score context
                if vulnerabilities:
                    if RISK_SCORE_THRESHOLD is not None:
                        print(f"\nHigh Severity Vulnerable Packages in '{project_name}' (Risk Score > {RISK_SCORE_THRESHOLD} or N/A):")
                    else:
                        print(f"\nHigh Severity Vulnerable Packages in '{project_name}':")
                    
                    for package, version, dep_type, risk_score, _ in vulnerabilities:
                        print(f"- {package} {version} {dep_type} (RiskScore: {risk_score})")
                else:
                    if RISK_SCORE_THRESHOLD is not None:
                        print(f"No high severity packages found for project '{project_name}' with risk score > {RISK_SCORE_THRESHOLD}.")
                    else:
                ...
                if license_issues:
                    if SHOW_LICENSE_ISSUES_ONLY_FOR_DIRECT:
                        print(f"\nRestricted License Packages (Direct Dependencies Only) in '{project_name}':")
                    else:
                        print(f"\nRestricted License Packages in '{project_name}':")
                    for package, version, dep_type, risk_score, _ in license_issues:
                        print(f"- {package} {version} {dep_type} (RiskScore: {risk_score})")
                else:
                    if SHOW_LICENSE_ISSUES_ONLY_FOR_DIRECT:
                        print(f"No restricted license packages (direct dependencies) found for project '{project_name}'.")
                    else:
...
def SCA_scan_packages(project_name, zip_manifest_file, team_name=None):
    ...
    # Print license issue filtering setting
    if SHOW_LICENSE_ISSUES_ONLY_FOR_DIRECT:
        print("License issue filtering: Direct dependencies only")
    else:
        print("License issue filtering: All dependencies (direct and transitive)")
...
def collect_package_references_from_project(csproj_path, visited=None):
    """Recursively collect PackageReference entries from a csproj and its ProjectReferences.
    Returns a list of dicts with 'name' and 'version' keys."""
    if visited is None:
        visited = set()

    csproj_path = os.path.normpath(os.path.abspath(csproj_path))
    ...
    try:
        ...
        for item_group in root.findall(".//ItemGroup"):
            ...
            for proj_ref in item_group.findall("ProjectReference"):
                ref_include = proj_ref.get('Include')
                if ref_include:
                    # Normalize Windows-style backslashes from the Include attribute
                    ref_path = os.path.normpath(os.path.join(base_dir, ref_include.replace('\\', os.sep)))
                    if os.path.isfile(ref_path):
                        print(f"DEBUG: Following ProjectReference: {ref_path}")
                        packages.extend(collect_package_references_from_project(ref_path, visited))
                    else:
                        print(f"DEBUG: ProjectReference not found on disk, skipping: {ref_path}")
    except Exception as e:
        print(f"Error collecting packages from {csproj_path}: {e}")
...
def process_csproj(file_path, temp_dir):
    try:
        ...
        # Collect and inline packages from ProjectReferences
        base_dir = os.path.dirname(os.path.abspath(file_path))
        merged_packages = {}  # name -> version, to deduplicate
        project_ref_item_groups = []

        for item_group in root.findall(".//ItemGroup"):
            proj_refs = item_group.findall("ProjectReference")
            if proj_refs:
                for proj_ref in proj_refs:
                    ...
                    if ref_include:
                        ref_path = os.path.normpath(os.path.join(base_dir, ref_include.replace('\\', os.sep)))
                        if os.path.isfile(ref_path):
                            print(f"DEBUG: Inlining packages from ProjectReference: {ref_path}")
                            for pkg in collect_package_references_from_project(ref_path):
                                if pkg['name'] not in merged_packages:
                                    merged_packages[pkg['name']] = pkg['version']
                        else:
                            print(f"DEBUG: ProjectReference not found, skipping: {ref_path}")
                    item_group.remove(proj_ref)
                    modified = True
                # Track empty ItemGroups for possible cleanup
                if len(item_group) == 0:
                    project_ref_item_groups.append(item_group)

        # Remove now-empty ItemGroups that only held ProjectReferences
        for ig in project_ref_item_groups:
            parent = root.find(".//*[.='']")
            # Walk tree to find parent
            for candidate in root.iter():
                if ig in list(candidate):
                    candidate.remove(ig)
                    break

        # Add merged packages as a new ItemGroup if any were collected
        if merged_packages:
            # Remove any packages that are already directly referenced in this csproj
            existing_names = {
                pkg_ref.get('Include')
                for item_group in root.findall(".//ItemGroup")
                for pkg_ref in item_group.findall("PackageReference")
            }
            new_refs = {name: ver for name, ver in merged_packages.items() if name not in existing_names}
            if new_refs:
                new_item_group = ET.SubElement(root, 'ItemGroup')
                for name, version in new_refs.items():
                    pkg_el = ET.SubElement(new_item_group, 'PackageReference')
                    pkg_el.set('Include', name)
                    if version:
                        pkg_el.set('Version', version)
                print(f"DEBUG: Inlined {len(new_refs)} package(s) from ProjectReference(s) into {os.path.basename(file_path)}")
                modified = True
...
# Fixed function to process requirements.txt files
def process_requirements_txt(file_path, temp_dir):
    try:
        ...
        for line in lines:
            ...
            # Parse package name and version - FIXED to handle ~= operator
            if '==' in line:
                parts = line.split('==')
                pkg_name = parts[0].strip()
                pkg_version = parts[1].strip()
            elif '~=' in line:  # ADD THIS: Handle compatible release operator
                parts = line.split('~=')
                pkg_name = parts[0].strip()
                pkg_version = parts[1].strip()
            elif '>=' in line:
                ...
                pkg_name = parts[0].strip()
                pkg_version = parts[1].strip()
            elif '>' in line:  # ADD THIS: Handle > operator
                parts = line.split('>')
                pkg_name = parts[0].strip()
                pkg_version = parts[1].strip()
            elif '<' in line:  # ADD THIS: Handle < operator
...
def zip_folder(folder_to_zip, output_folder='src', remove_dev_dependencies=True, exclude_folders=None, include_folders=None):
    try:
        ...
        # Process include_folders parameter
        if include_folders is None:
            include_folders = []
        elif isinstance(include_folders, str):
        ...
        include_folders_set = {folder.strip().lower() for folder in include_folders}
        
        # If include_folders is specified, it takes precedence over exclude_folders
        if include_folders_set:
            print(f"Including only folders: {', '.join(include_folders_set)}")
            print("Note: include_folders takes precedence over exclude_folders")
        elif exclude_folders_set:
        ...
        try:
            ...
            for root, dirs, files in os.walk(folder_to_zip):
                # Filter directories based on include_folders or exclude_folders
                if include_folders_set:
                    # If include_folders is specified, we need to allow the walk to continue
                    # until it finds the target folders, so we don't filter dirs here
                    pass
                else:
                    # Otherwise, filter out excluded directories from the walk
            ...
            with zipfile.ZipFile(zip_file_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
                for root, dirs, files in os.walk(folder_to_zip):
                    ...
                    if include_folders_set:
                        # If include_folders is specified, we need to allow the walk to continue
                        # until it finds the target folders, so we don't filter dirs here
                        # Instead, we'll filter at the file processing level
                        pass
                    else:
                        # Otherwise, filter out excluded directories from the walk
                        dirs[:] = [d for d in dirs if d.lower() not in exclude_folders_set]
                    ...
                    # Skip processing files if the current directory itself should be excluded
                    current_folder_name = os.path.basename(root).lower()
                    if include_folders_set:
                        # If include_folders is specified, only process if current folder is in the include list
                        # Skip the root directory entirely when using include_folders
                        if root == folder_to_zip:
                            print(f"DEBUG: Skipping root directory when using include_folders: {root}")
                            continue
                        
                        # Check if any of the include_folders is in the current path
                        # This allows for nested folder structures like nexus/npm/meitar
                        path_parts = [part.lower() for part in root.split(os.sep)]
                        has_included_folder = any(folder in path_parts for folder in include_folders_set)
                        
                        if not has_included_folder:
                            print(f"DEBUG: Skipping folder not in include list: {root}")
                            continue
                    else:
                        # Otherwise, skip if current folder is in exclude list
...
def main():
    ...
    try:
        ...
        parser.add_argument("--include_folders", nargs='*', default=[], help="Optional: List of folder names to include in scanning (opposite of exclude_folders). If specified, only these folders will be scanned (e.g., --include_folders src lib)")
        parser.add_argument("--additional_exclude_packages", default="", help="Optional: Path to additional JSON file with packages to exclude (will be merged with default excluded-packages.json)")
        ...
        additional_exclude_packages = args.additional_exclude_packages
        ...
        # Load default exclude packages JSON file
        if package_exclude_json:
            try:
                load_exclude_packages(package_exclude_json)
            except Exception as e:
                print(f"Error loading default exclude packages JSON: {e}")
                sys.exit(1)

        # Load additional exclude packages JSON file if provided
        if additional_exclude_packages and additional_exclude_packages.strip():
            try:
                load_exclude_packages(additional_exclude_packages)
            except Exception as e:
                print(f"Error loading additional exclude packages JSON: {e}")
                sys.exit(1)

        print(f"Total packages to exclude: {len(EXCLUDE_PACKAGES)}")
        ...
        if additional_exclude_packages:
            print(f"Additional exclude packages file = {additional_exclude_packages}")

        zip_file_name = zip_folder(source_folder, 'src', exclude_folders=exclude_folders, include_folders=include_folders)
